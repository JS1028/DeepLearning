...
class CycleGAN(nn.Module):
    def __init__(self, CONFIG, test=False):
        ...
        self.model_PtoA, self.model_AtoP = Create_GAN.Generator(4, first_ch=1, last_ch=3), Create_GAN.Generator(4, first_ch=3, last_ch=1)
        
        ######################################
        ### Pretrained Generator (w/ 0206)
        PATH = "/data/JS/Virtual_Staining/parameters/CycleStain/CycleStain_R_amp/"
        self.model_PtoA.load_state_dict(torch.load(PATH + 'Pretrain_XtoY/PNG/Glr0.001_Dlr0.0001_GenXtoY_of_CycleStain_Ramp_02_1ch_NIR_png_256x256_1000epoch.pt'))
        self.model_AtoP.load_state_dict(torch.load(PATH + 'Pretrain_YtoX/PNG/Glr0.001_Dlr0.0001_GenYtoX_of_CycleStain_Ramp_02_1ch_NIR_png_256x256_1000epoch.pt'))
        
#         self.model_D_x.load_state_dict(torch.load(PATH + 'Pretrain_YtoX/Glr0.001_Dlr0.0001_DisX_of_CycleStain_Ramp_02_1ch_NIR_mat_256x256_1000epoch.pt'))
#         self.model_D_y.load_state_dict(torch.load(PATH + 'Pretrain_XtoY/Glr0.001_Dlr0.0001_DisY_of_CycleStain_Ramp_02_1ch_NIR_mat_256x256_1000epoch.pt'))
...
#         self.last_Conv = {'54': 'last_Conv'}
        ...
        self.lr_scheduler_D_x = torch.optim.lr_scheduler.StepLR(self.optimizer_D_x, step_size=20, gamma=0.9)
        self.lr_scheduler_D_y = torch.optim.lr_scheduler.StepLR(self.optimizer_D_y, step_size=20, gamma=0.9)
        self.lr_scheduler_G_x = torch.optim.lr_scheduler.StepLR(self.optimizer_G_x, step_size=20, gamma=0.9)
        self.lr_scheduler_G_y = torch.optim.lr_scheduler.StepLR(self.optimizer_G_y, step_size=20, gamma=0.9)
...
#         self.data_X_phase = data_X_phase
    ...
    def new_loss(self, data_X, fake_Y, data_Y, fake_X):
        fake_amp = fake_Y[:, 0, :, :].unsqueeze(1)
        
        
        amp = data_Y[:, 0, :, :].unsqueeze(1)
        amp = torch.cat([amp, amp, amp], dim=1)
        

        
        amp_loss = self.criterion_L1(fake_X, self.model_AtoP(amp))
        phase_loss = self.criterion_L1(fake_amp, self.model_PtoA(data_X)[:,0,:,:].unsqueeze(1))
...
#         self.loss_content = self.criterion_L2(self.fake_x_content, self.x_content) + self.criterion_L2(self.fake_y_content, self.y_content)
        ...
        self.loss_G = 1*self.loss_C + (self.loss_G_X + self.loss_G_Y) + 2*self.new_loss(self.data_X, self.fake_y, self.data_Y, self.fake_x)   # TV loss = 0
        # self.loss_G = 80*self.loss_C + (self.loss_G_X + self.loss_G_Y)
    ...
    def save_generator(self, ep):
        PATH = "/data/JS/Virtual_Staining/parameters/CycleStain/Ver3.NewLoss/Trial7/"
        torch.save(self.model_G_x2y.state_dict(), PATH + f'Trial 7_CycleStain_{str(ep)}ep_02-AIAlpha_R_png_256x256.pt')
    ...
    def to_device(self, DEVICE, model=False):
        ...
        self.model_PtoA, self.model_AtoP = self.model_PtoA.to(DEVICE), self.model_AtoP.to(DEVICE)
        self.model_PtoA, self.model_AtoP = self.model_PtoA.eval(), self.model_AtoP.eval()
        #########################################################
