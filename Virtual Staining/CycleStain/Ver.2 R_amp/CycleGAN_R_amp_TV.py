...
#         self.loss_content = self.criterion_L2(self.fake_x_content, self.x_content) + self.criterion_L2(self.fake_y_content, self.y_content)
        ######################################
        # TV loss
        self.loss_TV_fakeY = self.compute_total_variation_loss(self.fake_y, 1)
        self.loss_TV_fakeX = self.compute_total_variation_loss(self.fake_x, 1)
        self.loss_TV = self.loss_TV_fakeX + self.loss_TV_fakeY
        ######################################
        # Final Loss
        
#         self.loss_G = 80*self.loss_C + 0.1*self.loss_content + 1*(self.loss_G_X + self.loss_G_Y)
        
#         self.loss_G = self.loss_C + 0.02 * self.loss_TV + 0.01*self.loss_content + 2000*(self.loss_G_X + self.loss_G_Y)
#         self.loss_G = 70*self.loss_C + 1*self.loss_identity + self.loss_G_X + self.loss_G_Y
        self.loss_G = 5*self.loss_C + 0.005*self.loss_content + 2000*(self.loss_G_X + self.loss_G_Y)
         ######################################
