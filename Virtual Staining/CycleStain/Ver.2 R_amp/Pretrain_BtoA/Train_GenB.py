...
wandb.init(project='CycleStain - R amp pretrain YtoX',
           config=HPARAMS,
           name='500ep_02.sto_NIR_lrG(0.001)_lfD(0.0001)_256',
           # mode='disabled',
           notes=NOTES)
...
#             UnPaired_Phase_data.append(np.expand_dims(a[7,:,:], axis=0))
    ...
    model = GenYtoX_DisX.CycleGAN(HPARAMS)
...
#             model.set_input(phase[0], target[0])
            # model.set_input(target[0], phase[0])
#             print(f"trainA_min: {torch.min(train_dataA[0][:,0,:,:].unsqueeze(1))}")
#             print(f"trainA_max: {torch.max(train_dataA[0][:,0,:,:].unsqueeze(1))}")
