...
class CycleGAN(nn.Module):
    def __init__(self, CONFIG, test=False):
        ...
        self.CONFIG = CONFIG
        self.test = test
        self.model_G_x2y = Create_GAN.Generator(4, first_ch=3, last_ch=1)
        self.model_D_y = Create_GAN.Discriminator(1)
...
#         return self.loss_D_X+self.loss_D_Y, self.loss_G_X, self.loss_G_Y, self.loss_C, self.phase, self.data_Y, self.fake_x, self.fake_y, self.msSSIM_x2y, self.msSSIM_y2x
    ...
    def save_generator(self, ep):
        PATH = "/data/JS/Virtual_Staining/parameters/CycleStain/CycleStain_R_amp/Pretrain_YtoX/PNG/"
        torch.save(self.model_G_x2y.state_dict(), PATH + f'Glr0.001_Dlr0.0001_GenYtoX_of_CycleStain_Ramp_02_1ch_NIR_png_256x256_{str(ep)}epoch.pt')
        torch.save(self.model_D_y.state_dict(), PATH + f'Glr0.001_Dlr0.0001_DisX_of_CycleStain_Ramp_02_1ch_NIR_png_256x256_{str(ep)}epoch.pt')
