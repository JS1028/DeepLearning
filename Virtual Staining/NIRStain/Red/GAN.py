...
class Generator(nn.Module):
    def __init__(self):
        ...
        def gen_down_block(in_ch, out_ch):
            layers = [
                nn.Conv2d(in_ch, out_ch//2, kernel_size=3, stride=1, padding=1),  # NxN유지
        ...
        ###########################################################################
        def connection_block(in_ch, out_ch):
            layers = [
                nn.Conv2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1),  # NxN유지
                nn.BatchNorm2d(num_features=out_ch),
                nn.LeakyReLU(0.1),
        ...
        self.pooling = nn.AvgPool2d(kernel_size=2, stride=2) # H와 W 0.5배
...
#         self.down4 = gen_down_block(256, 512)
        self.connection = gen_down_block(256, 256)
        
        # 256x256 or 512x512
#         self.unpool1 = nn.ConvTranspose2d(512, 512, kernel_size=2, stride=2, padding=0, bias=True)

        # 600x600
        self.unpool1 = nn.ConvTranspose2d(256, 256, kernel_size=2, stride=2, padding=0, bias=True) # H와 W 2배
        self.unpool2 = nn.ConvTranspose2d(128, 128, kernel_size=2, stride=2, padding=0, bias=True)
        self.unpool3 = nn.ConvTranspose2d(64, 64, kernel_size=2, stride=2, padding=0, bias=True)
#         self.unpool4 = nn.ConvTranspose2d(64, 64, kernel_size=2, stride=2, padding=0, bias=True)

        self.up1 = gen_up_block(4 * 128)
        self.up2 = gen_up_block(4 * 64)
        self.up3 = gen_up_block(4 * 32)
...
#         add4 = self.point_wise_add(pool3, self.down4(pool3))
        ...
        latent = self.connection(self.pooling(add3))
#         print(latent.shape)
#         print(add3.shape)
        up1 = torch.cat((self.unpool1(latent), add3), dim=1)
        up1 = self.up1(up1)

        up2 = torch.cat((self.unpool2(up1), add2), dim=1)
        up2 = self.up2(up2)

        up3 = torch.cat((self.unpool3(up2), add1), dim=1)
        up3 = self.up3(up3)

#         up4 = torch.cat((self.unpool4(up3), add1), dim=1)
#         up4 = self.up4(up4)

        output = self.recover(up3)
...
#             nn.LeakyReLU(0.1),
        ...
        self.pooling = nn.AvgPool2d(kernel_size=19)
        # input: 600x600 -> kernel_size = 19
...
#     summary(gen, input_size=(1, 600, 600))
    
#     dis = Discriminator().to(device)
#     summary(dis, input_size=(3, 600, 600))
