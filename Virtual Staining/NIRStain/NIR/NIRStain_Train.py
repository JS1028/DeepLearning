...
NAME = "Trial 1: Mod1_02.sto&06.sto_S_NIR_mat_600x600"

NOTES = '02.sto_S_NIR_phase'
START_DATE = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")

wandb.init(project='NIRStain',
...
train_data, valid_data = data[:-7], data[-7:]

dataset = Make_Dataset(train_data)
valid_dataset = Make_Dataset(valid_data)

train_loader = DataLoader(dataset, batch_size=HPARAMS['BATCH_SIZE'], shuffle=False)
...
for epoch in range(epochs):
    ...
    for idx, data in enumerate(train_loader):
        ...
        print('---------')
        print(torch.min(phase[0]))
        print(torch.max(phase[0]))
        print()
        print(torch.min(rgb[0]))
        print(torch.max(rgb[0]))
        print('---------')
...
torch.save(generator.state_dict(), PATH + 'NIRStain_02_NIR_600x600.pt')